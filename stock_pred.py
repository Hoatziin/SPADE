...
Y_test=[]
for i in range (60, inputs_data.shape[0]):
    Y_test.append(inputs_data[i-60:i,0])
...
data=df.sort_index(ascending=True,axis=1)
new_dataset=pd.DataFrame(index=range(10223,18205),columns=['Date','Close'])
for i in range(10223,18205):
    new_dataset["Date"][i]=data['Date'][i]
    new_dataset["Close"][i]=data["Close"][i]
...
train_data=new_dataset[:4500]
valid_data=new_dataset[4500:]
valid_data.insert(1, "Predictions", predicted_closing_price, True)
plt.plot(train_data["Close"])
plt.plot(valid_data[['Close',"Predictions"]])
...
data=df.sort_index(ascending=True,axis=1)
new_dataset=pd.DataFrame(index=range(18206, 19586),columns=['Date','Close'])
for i in range(18206,19586):
    new_dataset["Date"][i]=data['Date'][i]
    new_dataset["Close"][i]=data["Close"][i]
